...
final class LoginContent extends Content{
	...
	protected $tag_keys = array('chgVmodeEdit','chgVmodeNormal','username','password','login','group','logout','forgot_pw');
	protected $tags = array('edit','normal','username','password','login','group','logout','forgot password?');
	...
	public function display(){
		...
		if($GLOBALS['user']->__get('id') != ANONYMOUS_USER_ID){
			...
			$user_info = '<p><b>'.$tag['username'].'</b> '.$GLOBALS['user']->__get('username').'<br/>
						<b>'.$tag['group'].'</b> '.$GLOBALS['user']->__get('primary_user_group_name').'</p>';
					
			return '<div id="loginbox">
						'.$vmode_switch.$user_info.$this->logout_form()->getHtml().'
					</div>';
		} else {
