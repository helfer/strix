<?php
class ibo2013AdditionalJuryRegistrationContent extends Content {
	...
	protected $visitor_rebate = 200;
	protected $cheaper_bed_rebate = 500;
	protected $catname = 'additional jury member';
	protected $catname_caps = 'Additional Jury Member';
	...
	public function display(){	
		...
		if($hasHistory){
			...
			$xhtml.='<p class="text"><img src="/webcontent/images/exclamation.png" style="float:left;padding-right:10px;"><b>NOTE:</b> You can only cancel registrations for which no names have been given yet. Please remove names first (see <a href="http://www.ibo2013.org/Participate/registration/provide_names/">here</a>).</p>';
	...
	public function writeObserverFees(&$sql, &$xhtml){
		...
		$xhtml.='<p class="text">The indicated fees are valid if an '.$this->catname.' is registered before or on the date specified in the table below. The relevant time zone is the Central European Time (CET).</p>';
		...
		$xhtml.='<p class="text"><b>Visitors:</b> Jury members are expected to be present and participate in all jury sessions. However, several delegations registered accompanying persons (non-jury members) as additional jury members in the past. To avoid this confusion, we offer the possibility to register these accompanying persons as visitors, for a fee which is CHF '.$this->visitor_rebate.' cheaper than the current base rate of an additional jury member. Visitors will be welcomed at the opening and closing ceremony and on all parties and excursions, but will not be granted access to the jury room nor be provided meals during the jury sessions (lunch on Monday through Wednesday, dinner on Monday and Wednesday). The number of available hotel rooms is limited and priority will be given to jury members. To register a person as visitor, choose "Visitor" from the "Category" drop down menu below.</p>';			
	}
	...
	public function process_observer_registration_form(){
		...
		if($values['category']=='Jury Budget'){
	...
	protected function get_current_fee(&$sql, $type){
		...
		switch($type){
			...
			case 'Jury Budget': return $this->currentFee - $this->cheaper_bed_rebate; break;
			case 'Jury': return $this->currentFee; break;			
		}		
	}
	...
	protected function observer_registration_form($vector){	
		...
		$res=$this->readCheaperObserverbedsFromDB($sql);
		$categories=array();
		$categories['Jury']='Jury (CHF '.$this->get_current_fee($sql, 'Jury').')';
		if($res['available']>0) $categories['Jury Budget']='Jury Budget (CHF '.$this->get_current_fee($sql, 'Jury Budget').')';	
		$categories['Visitor']='Visitor (CHF '.$this->get_current_fee($sql, 'Visitor').')';
	...
	protected function observer_cancellation_form($vector){	
		...
		//-> how many have been signed up? = Num per category - signed up. BUT: The Jury category may have one additional due to the team
		...
		foreach($num as $id=>$n){
			if($n['category']=='Jury Budget' && isset($types['Jury Budget'])) $types['Jury Budget']['num']-=$n['num'];
			if($n['category']=='Visitor' && isset($types['Visitor'])) $types['Visitor']['num']-=$n['num'];
			if($n['category']=='Jury' && isset($types['Jury'])){
				$types['Jury']['num']-=$n['num'];
				//maybe one Jury is from Team Registration?
				$query='select num_jury from ibo2013_team_registration where country_id='.$this->getMyCountryId($sql);
				$sql->start_transaction(); 
				$numjury=$sql->simpleQuery($query);
				$ok = $sql->end_transaction(); 
				$types['Jury']['num']+=($numjury[0]['num_jury']-1);
				//if($numjury[0]['num_jury']==2) ++$types['Jury']['num'];
			}
		}		
		
		//make list to choose from
		$categories=array();
		if(isset($types['Jury']) && $types['Jury']['num']>0) $categories['Jury']='Jury (Cancellation fee CHF '.$types['Jury']['fee'].')';
		if(isset($types['Jury Budget']) && $types['Jury Budget']['num']>0) $categories['Jury Budget']='Jury Budget (Cancellation fee CHF '.$types['Jury Budget']['fee'].')';
		if(isset($types['Visitor']) && $types['Visitor']['num']>0) $categories['Visitor']='Visitor (Cancellation fee CHF '.$types['Visitor']['fee'].')';
